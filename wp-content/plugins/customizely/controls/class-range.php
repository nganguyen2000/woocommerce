...
class Range extends Control {
	 ...
	 * @var string
	 */
	public $type = 'cmly_range';

	/**
	 * Minimum
	 *
	 * Holds minimum value of range input.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 * Maximum
	 *
	 * Holds maximum value of range input.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 * Step
	 *
	 * Holds step value of range input.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 * No Unit
	 *
	 * Holds is unit will work or not work with range input.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 * Default Unit
	 *
	 * Holds default unit for range input.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 * Default Unit
	 *
	 * Holds supported units for range input.
	...
	public function input_template() {
		...
		<#
			var number = 0;
			if(!_.isEmpty(data.value) && !_.isNull(data.value) && !_.isArray(data.value)){
				number = parseFloat( data.value );
			}

			var numberInputType = data.no_unit ? 'number' : 'text';
		#>
		<input class="customizely-range-input" type="range" min="{{{ data.min }}}" max="{{{ data.max }}}" step="{{{ data.step }}}" value="{{{ number }}}" />
...
function register_range() {
	customizely_register_control( 'cmly_range', 'Customizely\Controls\Range' );
}
add_action( 'customizely_after_customize_init', 'Customizely\Controls\register_range' );
 ...
 * @return void
 */
function register_range_settings() {
	...
	array_splice( $default_options[ $values_index['tab'] ]['options'], 0, 0, $range_options );
...
add_action( 'customizely_loaded', 'Customizely\Controls\register_range_settings' );

/**
 * Process Range Arguments
 *
 * This function will process special range arguments for customize.
 *
 ...
 * @return array
 */
function process_range_args( $args, $values ) {
...
add_filter( 'customizely_processed_cmly_range_control_args', 'Customizely\Controls\process_range_args', 10, 2 );
